...
def shop(client, utterance, user, **kw):
    return client.post("/api/shop",
                       json={"utterance": utterance, "user_id": user, **kw}).json()
...
def test_a_step_up_can_actually_be_confirmed(client):
    ...
    first = shop(client, "buy whisky under 2000", "usr_a")
    ...
    ok = shop(client, "buy whisky under 2000", "usr_a", human_confirms=True)
...
def test_declining_a_step_up_moves_no_money(client):
    shop(client, "buy whisky under 2000", "usr_b")
    no = shop(client, "buy whisky under 2000", "usr_b", human_confirms=False)
...
def test_one_visitor_does_not_spend_another_visitors_limit(client):
    ...
    for i in range(15):
        shop(client, "buy a yoga mat under 1500", f"usr_crowd{i}")
    mine = shop(client, "buy a yoga mat under 1500", "usr_arrives_late")
...
def test_exposure_forgets_yesterday(client, monkeypatch):
    ...
    shop(client, "buy a yoga mat under 1500", "usr_c")
    api_mod.get_app().db.execute(
        "UPDATE payments SET created_at=? WHERE user_id='usr_c'",
        ((NOW - timedelta(days=3)).isoformat(),))
    exp = api_mod._exposure(api_mod.get_app(), "usr_c")
...
def test_repeating_an_utterance_still_leads_to_a_payable_order(client):
    ...
    first = shop(client, "buy a yoga mat under 1500", "usr_d")
    again = shop(client, "buy a yoga mat under 1500", "usr_d")
...
def test_checkout_never_leaks_the_secret(client, monkeypatch):
    ...
    d = shop(client, "buy a yoga mat under 1500", "usr_e")
